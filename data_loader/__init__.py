...
class DataHandler:
    ...
    def reload(self, split, downsample=None):
        ...
        if split == "train":
            self.c2ws = split_dataset.poses[::4]
            self.rays, self.rgbs = split_dataset.all_rays[::4], split_dataset.all_rgbs[::4]
            self.alphas = getattr(
                split_dataset, "all_alphas", torch.ones_like(self.rgbs[..., 0:1])[::4]
            )
        else:
